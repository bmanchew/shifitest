 ...
 * 
 * Usage: npx tsx check-asset-reports.ts
 */
  ...
  try {
    ...
    for (const merchant of completedMerchants as PlaidMerchant[]) {
      ...
      if (merchantReports && merchantReports.length > 0) {
        assetReports.push(...merchantReports as AssetReport[]);
      }
    }
    ...
    // Process each report
    assetReports.forEach((report: AssetReport) => {
    ...
    if (reportsByStatus.pending.length > 0) {
      console.log('\nPending reports:');
      reportsByStatus.pending.forEach((report: AssetReport) => {
    ...
    if (reportsByStatus.ready.length > 0) {
      console.log('\nReady reports:');
      reportsByStatus.ready.forEach((report: AssetReport) => {
        console.log(`  - Report ID: ${report.assetReportId}, Created: ${report.createdAt}`);
      });
    }
    ...
    if (reportsByStatus.error.length > 0) {
      console.log('\nError reports:');
      reportsByStatus.error.forEach((report: AssetReport) => {
