...
const { merchantProgramAgreements, merchantPrograms, merchants } = require('@shared/schema');
...
async function main() {
  ...
  try {
    ...
    if (agreement.externalTemplateId) {
      ...
      // Check if the agreement data is available (it might be base64 encoded)
      if (!agreement.data) {
        console.error('Agreement data is missing or empty. Cannot create template.');
        process.exit(1);
      }
