#!/usr/bin/env python3
"""
Script to convert text to speech using Google Text-to-Speech (gTTS)
This script is an alternative to run_csm.py that uses gTTS for high-quality speech
"""
...
# Import the gtts generator
try:
    from gtts_generator import Segment, load_gtts_model
except ImportError as e:
    print(json.dumps({
        "success": False,
        "error": f"Could not import gtts_generator module: {str(e)}"
    }))
    sys.exit(1)
...
# Constants
SAMPLE_RATE = 24000  # The expected sample rate for audio output
...
def generate_audio(text, speaker_id=0, output_path=None):
    """
    Generate audio from text using the gTTS model
    ...
    try:
        ...
        model = load_gtts_model(device="cpu")
        ...
        audio = model.generate(
            ...
            temperature=0.9,  # Not used for gTTS
            topk=50  # Not used for gTTS
        )
        
        # Determine the output filename
        if output_path is None:
            filename = f"gtts_{uuid.uuid4().hex[:8]}_{speaker_id}.wav"
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Convert text to speech using Google Text-to-Speech')
