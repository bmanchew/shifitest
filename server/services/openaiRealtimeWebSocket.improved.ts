...
export class OpenAIRealtimeWebSocketService {
  ...
  private async handleCreateSession(client: ClientConnection, data: any): Promise<void> {
    ...
    try {
      ...
      const sessionResponse = await openaiRealtimeService.createSession({
        model: 'gpt-4o',
        voice: data.voice || 'alloy',
        instructions: data.instructions || `You are a helpful assistant named Financial Sherpa.`
      });
      
      const { sessionId, sessionUrl, token } = sessionResponse;
  ...
  private async reconnectToOpenAI(client: ClientConnection): Promise<void> {
    ...
    try {
      ...
      // Get a new token for the existing session
      const sessionResponse = await openaiRealtimeService.getSessionToken(sessionId);
      
      // Connect to OpenAI with the new token
      await this.connectToOpenAI(client, sessionResponse.sessionUrl, sessionResponse.token);
    } catch (error) {
      logger.error({
